...
namespace EasySII.Watcher.Xml.SIICreators
{
    ...
    /// </summary>
    public class FEIvaBase : ISIICreator
	{
		...
		public Envelope GetEnvelope()
		{
			...
			loteDeFacturasEmitidas.CommunicationType = CommunicationType.A0;

			loteDeFacturasEmitidas.ARInvoices.Add(GetARInvoice());

			return loteDeFacturasEmitidas.GetEnvelope(true);
		...
		public virtual Envelope GetDeleteEnvelope()
		{
			...
			ARInvoicesDeleteBatch loteDeBorradoFacturasEmitidas = new ARInvoicesDeleteBatch();
			loteDeBorradoFacturasEmitidas.Titular = titular;

			loteDeBorradoFacturasEmitidas.ARInvoices.Add(GetARInvoice());

			return loteDeBorradoFacturasEmitidas.GetEnvelope();
		}
		...
		/// <summary>
		/// Devuelve una factura recibida representada por un objeto ARinvoice.
		/// </summary>
		/// <returns>Devuelve un obejero APInvoice conformado con los 
		/// datos provenientes de SAP.</returns>
		internal virtual ARInvoice GetARInvoice()
		{
			ARInvoice facturaEmitida = new ARInvoice(); // Factura emitida

			SetInvoiceIssueDate(facturaEmitida);

			facturaEmitida.InvoiceNumber = DocumentNumber; // El número de factura

			if (GrossAmount < 0 || _InnerDocDataSet[0][19] != "")
			{
				facturaEmitida.InvoicesRectified = new List<InvoiceRectified>();

				foreach (var invoiceLine in _InnerDocDataSet)
				{
					if (!string.IsNullOrEmpty(invoiceLine[14]) && 
                        !string.IsNullOrEmpty(invoiceLine[15]))
					{
						facturaEmitida.InvoicesRectified.Add(new InvoiceRectified()
						{
							RectifiedInvoiceNumber = invoiceLine[14],
							RectifiedIssueDate = Convert.ToDateTime(invoiceLine[15])
						});
					}
				}

                if (facturaEmitida.InvoicesRectified.Count == 0)
                    facturaEmitida.OperationIssueDate = Convert.ToDateTime(_InnerDocDataSet[0][15]);

                InvoiceType invoiceType = 0;

                if (Enum.TryParse<InvoiceType>(_InnerDocDataSet[0][19], out invoiceType))
                    facturaEmitida.InvoiceType = invoiceType;
                else
                    facturaEmitida.InvoiceType = InvoiceType.R1; // Rectificativa normal                

			}
			else
			{
				facturaEmitida.InvoiceType = InvoiceType.F1; // Factura normal
			}

			facturaEmitida.ClaveRegimenEspecialOTrascendencia = ClaveRegimenEspecialOTrascendencia.RegimenComun;
			...
			LoadTaxes(facturaEmitida);

			foreach (var kvp in facturaEmitida.TaxesOutputs)
				facturaEmitida.GrossAmount += kvp.Value[0] + kvp.Value[1];

			if (facturaEmitida.GrossAmount == 0)
		...
		/// </summary>
		internal virtual void SetInvoiceIssueDate(ARInvoice facturaEmitida)
		{
		...
		internal virtual void LoadTaxes(ARInvoice facturaEmitida)
		{
			foreach (var row in _InnerDocDataSet)
			{
				...
				if (!string.IsNullOrEmpty($"{row[9]}") && !string.IsNullOrEmpty($"{row[10]}"))
				{
					...
					facturaEmitida.AddTaxOtuput(taxRate, taxBase, taxAmount);
				}
			}
		}
